...
#         actor_role_key (str): definitions.ROLES で定義されたアクターのロールキー。
#         pds_user_prompts (list[str]): PDSがユーザー役として発話するプロンプトのリスト。
#         seed (int, optional): 再現性のためのシード値。

#     Returns:
#         list: 全対話履歴 (Actor内のmessagesリスト)。エラー時はNone。
#     """
#     actor_system_prompt = ROLES[actor_role_key]
...
#     print(f"role message is :'{actor_system_prompt}'")

#     # pds_user_prompts に従って対話を実行
#     for i, user_utterance in enumerate(pds_user_prompts):
#         current_turn = i + 1
#         print(f"\n[ターン {current_turn} - PDS (ユーザー役)]")

#         enhanced_user_utterance = f"あなたは「{ROLES[actor_role_key][:30]}...」という役割です。次の発言に答えてください：{user_utterance}"

#         print(f"発話: {enhanced_user_utterance}")

#         llm_response = pds_actor.ask(enhanced_user_utterance)
#         # llm_response = pds_actor.ask(user_utterance)


#         if llm_response is None:
#             print(f"エラー: ターン {current_turn} でLLMからの応答がありませんでした。セッションを中断します。")
#             # Actorのメッセージ履歴には、エラー発生前のものは残っている
#             return pds_actor.messages

#         print(f"\n[ターン {current_turn} - LLMアクター ({actor_role_key})]")
#         print(f"応答: {llm_response}")
#         print("--------------------------")

#     print("\n--- PDSセッション終了 ---")
#     print("最終的な対話履歴:")
#     for message_idx, message in enumerate(pds_actor.messages):
#         print(f"  [{message_idx}] {message['role']}: {str(message['content'])[:120]}...")
