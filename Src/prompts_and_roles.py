...
対話相手（部長）の特徴
あなたの部長は、経験豊富ですが、少し気難しいことで知られています。彼の思考にはいくつかの「クセ」があります。あなたの提案に対し、彼は以下のような反応を示す可能性が高いでしょう。

【体裁へのこだわり】: 提案の本質的な価値よりも、企画書の見た目や些細なミスを指摘してくるかもしれません。
【コスト意識】: すぐに「で、儲かるのか？」「予算は？」といった、費用やリソースの話に脱線させようとします。
【過去への固執】: あなたの革新的な提案を、彼が知っている過去の無関係な失敗例と結びつけ、否定的に見てくる傾向があります。
【リスク回避】: 「失敗したらどうするんだ」という責任問題や、他部署からの反対を過度に気にする発言が目立ちます。
攻略のヒント
この対話を成功させる（＝承認を得る）ためには、以下の点を意識してください。

冷静さを保つ: 相手のピントはずれな指摘に、感情的にならないでください。これはあなたの対応力と思考の深さを試すテストです。
一度は受け止める: 相手の指摘を頭ごなしに否定せず、「おっしゃる通りですね、その点については…」と一度は受け止める姿勢を見せましょう。
巧みに軌道修正する: 相手の懸念に答えた上で、「そして、その上で最も重要なのが…」と、巧みに議論を本題へと引き戻してください。
説明を多角化する: 一つの説明で理解されなくても諦めないでください。言葉を変え、比喩を使い、異なる角度からアプローチすることで、相手の理解の扉を開きましょう。
